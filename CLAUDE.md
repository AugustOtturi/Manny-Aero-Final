...
- `/contact` — formulario embebido de **Zoho Forms** (iframe, decisión del cliente ago 2026) + contact cards. El formulario propio original está respaldado en `src/backup/contact-original.astro`; para revertir, restaurar ese archivo y quitar `forms.zohopublic.com` del `frame-src` en `src/lib/csp.ts`. Los envíos van directo al Zoho del cliente — **no** pasan por `/api/contact`, no se guardan en `leads` ni mandan email a ops (el FinalCTA y el email gate sí siguen usando `/api/contact` y `/api/gate`).
...
- **Astro 5**, `output: "server"` con adapter `@astrojs/node` (modo `middleware`). Las 7 páginas 100% estáticas (`about`, `catering`, `isbah`, `founder`, `ground-handling`, `contact`, `404`) tienen `export const prerender = true` — se generan como HTML plano en el build (`build.format: "file"` → `about.html`, servido por el `express.static` de `server.mjs` con `extensions: ["html"]` y `Cache-Control: no-cache`). El resto (index, permits, news, admin) se sirve por request porque lee de la DB. Ojo: si una página prerenderizada empieza a leer de la DB, hay que quitarle el `prerender` o quedará congelada al build. `src/middleware.ts` se salta el chequeo de sesión en rutas prerenderizadas (`context.isPrerendered`).
- **TypeScript** (strict)
...
- Endpoints: `GET/POST /api/admin/map/categories`, `PUT/DELETE /api/admin/map/categories/:id`, `GET/POST /api/admin/map/airports`, `PUT/DELETE /api/admin/map/airports/:id`, `POST /api/admin/map/airports/:id/file`. Validación zod en `src/lib/server/schemas/map.ts`; PDFs via `src/lib/server/airportFiles.ts` (solo `.pdf`, 15 MB, magic bytes) a `UPLOADS_DIR/airport-files/`. Los PDFs legacy de `public/files/airports/` son compartidos por varios pins y **nunca** se borran. En disco los uploads llevan prefijo de timestamp (anti-colisión); el nombre original del archivo se guarda en la columna `pdf_name` y se usa como texto del link en el admin y como atributo `download` del popup público (los legacy tienen `pdf_name` NULL y descargan con el basename de la URL).
